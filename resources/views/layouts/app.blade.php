...
<head>
    ...
    <title>@yield('title', 'Website Informasi')</title>
...
<div class="min-h-screen flex flex-col">
    <div class="bg-neutral-950 border-b border-neutral-800 text-[10px] uppercase tracking-[0.2em] font-semibold text-neutral-500 px-4 py-2.5 flex flex-wrap items-center justify-between gap-2 z-20 font-mono">
        ...
        <div class="flex items-center gap-4 text-neutral-500">
            <span>UTC Terminal: {{ now()->toDateString() }}</span>
    ...
    <header class="bg-neutral-950/80 backdrop-blur-md border-b border-neutral-800 sticky top-0 z-30">
        <div class="w-full max-w-7xl mx-auto px-4 py-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <nav class="flex flex-wrap items-center gap-1">
                <a href="{{ route('home') }}" class="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-bold uppercase tracking-wider transition-all bg-neutral-800 text-blue-400 border border-neutral-700">
                    <span>Berita</span>
                </a>
                @if($adminUser)
                    <a href="{{ route('admin.index') }}" class="px-3 py-1.5 rounded text-xs font-black uppercase tracking-wider transition-all cursor-pointer bg-neutral-900 border border-neutral-800 text-blue-450 hover:bg-neutral-800">Admin CMS</a>
                ...
                @else
                    <a href="{{ route('login') }}" class="flex items-center gap-1.5 px-3.5 py-1.5 rounded text-xs font-bold uppercase tracking-widest bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 hover:border-neutral-700 transition-all text-neutral-350 hover:text-white">
    ...
    @if(session('success') || session('error'))
        <div class="w-full max-w-7xl mx-auto px-4 pt-4">
            <div class="{{ session('success') ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : 'bg-red-500/10 border-red-500/20 text-red-400' }} border text-xs rounded p-3 font-mono">
