...
@section('title', 'Website Informasi')
    ...
    <div class="bg-neutral-900/45 border border-neutral-800 rounded-2xl p-8 md:p-12 flex flex-col lg:flex-row gap-8 items-center justify-between relative overflow-hidden shadow-lg">
        ...
        <div class="w-full lg:w-72 border-t lg:border-t-0 lg:border-l border-neutral-800 pt-6 lg:pt-0 lg:pl-10 space-y-6 shrink-0">
            <div>
                ...
                <div class="text-3xl font-black text-neutral-150">{{ $articles->count() }}</div>
            ...
            <div>
                ...
                <div class="text-3xl font-black text-neutral-150">{{ count($categories) }}</div>
                <div class="text-[9px] font-mono text-blue-400 mt-2 uppercase block font-bold">filter berita tersedia</div>
            </div>
        </div>
    </div>

    <form method="GET" action="{{ route('home') }}" class="bg-neutral-900/60 border border-neutral-800 rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
            @foreach(['All', ...$categories] as $category)
                <button name="category" value="{{ $category }}" class="py-1.5 px-3 rounded text-[11px] font-bold uppercase tracking-wider whitespace-nowrap transition-all cursor-pointer {{ $selectedCategory === $category ? 'bg-neutral-800 text-blue-400 border border-neutral-700' : 'bg-transparent text-neutral-450 hover:text-neutral-200' }}">
                    {{ $category }}
                </button>
            @endforeach
        </div>
        <div class="flex items-center bg-neutral-950 border border-neutral-850 rounded px-3 py-1.5 w-full md:w-72">
            ...
            <input name="q" type="text" placeholder="Cari berita atau advis siber..." class="w-full bg-transparent text-neutral-150 placeholder:text-neutral-600 text-xs py-0.5 px-2 focus:outline-none" value="{{ $currentSearch }}">
        </div>
    </form>
                ...
                <article class="bg-neutral-900/25 hover:bg-neutral-900/60 border border-neutral-800/80 hover:border-neutral-700 rounded-2xl overflow-hidden shadow-lg transition-all flex flex-col cursor-pointer group" x-data="{ open: false }">
                    <button type="button" @click="open = true" class="text-left">
                        <div class="aspect-[16/9] bg-neutral-950 border-b border-neutral-800/60 overflow-hidden relative">
                            <div class="absolute top-3 left-3 z-10 bg-neutral-950/90 text-neutral-300 text-[9px] border border-neutral-800 px-2.5 py-0.5 rounded font-bold uppercase tracking-wider">{{ $article->category }}</div>
                            <img
                        ...
                        <div class="p-5 flex-1 flex flex-col justify-between">
                            <div class="space-y-3">
                                <div class="flex items-center gap-2 text-neutral-500 text-[10px] font-mono">
                                    <span class="font-bold uppercase text-neutral-400">{{ $article->author }}</span>
                                    <span>&bull;</span>
                                    <span>{{ $article->published_date?->format('Y-m-d') }}</span>
                                </div>
                                <h4 class="text-base sm:text-lg font-serif italic text-neutral-100 group-hover:text-blue-400 transition-colors leading-snug line-clamp-2">{{ $article->title }}</h4>
                                <p class="text-neutral-400 text-xs leading-relaxed line-clamp-3 font-serif">{{ $article->summary }}</p>
                            </div>
                            <div class="flex flex-wrap gap-1.5 pt-4 border-t border-neutral-900/50 mt-4">
                                @foreach(array_slice($article->tags ?? [], 0, 3) as $tag)
                                    <span class="text-[9px] font-mono bg-neutral-950 text-neutral-500 border border-neutral-850 rounded px-1.5 py-0.5">#{{ $tag }}</span>
                                @endforeach
                            </div>
                        </div>
                    </button>
