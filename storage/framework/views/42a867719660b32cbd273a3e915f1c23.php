...
<div class="fixed inset-0 bg-neutral-950/85 backdrop-blur-sm flex items-center justify-center p-4 z-40">
    <div class="w-full max-w-md bg-neutral-900 border border-neutral-800 rounded-2xl p-6 md:p-8 shadow-2xl relative">
        <a href="<?php echo e(route('home')); ?>" class="absolute top-4 right-4 text-neutral-400 hover:text-white transition-colors cursor-pointer">X</a>
        ...
        <?php if(session('error')): ?>
            <div class="bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-lg p-3 mb-4 font-mono"><?php echo e(session('error')); ?></div>
        <?php endif; ?>
        <form method="POST" action="<?php echo e(route('login.store')); ?>" class="space-y-4 font-mono text-[11px]">
            <?php echo csrf_field(); ?>
            <div>
                <label class="block font-bold text-neutral-400 mb-1.5 uppercase tracking-widest">Username</label>
                <input name="username" type="text" required class="w-full bg-neutral-950 border border-neutral-800 focus:border-blue-500/50 rounded py-2 px-3 text-xs text-white placeholder:text-neutral-700 font-sans focus:outline-none transition-colors" placeholder="Masukkan username admin..." value="<?php echo e(old('username')); ?>">
            </div>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/vol15_2/infinityfree.com/if0_42129557/htdocs/resources/views/auth/login.blade.php ENDPATH**/ ?>