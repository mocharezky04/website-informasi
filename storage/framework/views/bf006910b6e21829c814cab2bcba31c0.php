<?php $__env->startSection('title', $isEdit ? 'Edit Artikel Siber' : 'Tambah Artikel Siber'); ?>
...
?>
<meta name="openrouter-key" content="<?php echo e(config('services.openrouter.api_key')); ?>">
<meta name="openrouter-categories" content="<?php echo e(implode(',', \App\Http\Controllers\ArticleController::CATEGORIES)); ?>">
<div class="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-6 md:p-8 shadow-xl relative" x-data="adminArticleForm()">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-neutral-800 pb-5 mb-5 gap-3">
        <div>
            <h3 class="text-lg font-serif italic text-white"><?php echo e($isEdit ? 'Edit Advis Keamanan' : 'Tulis Artikel Siber Baru'); ?></h3>
            <p class="text-xs text-neutral-450 mt-1 font-serif">Form artikel mendukung Markdown, gambar eksternal, upload gambar, tag, dan asisten draf <?php echo e($activeAiLabel); ?>.</p>
        </div>
        <a href="<?php echo e(route('admin.index')); ?>" class="flex items-center gap-1.5 bg-neutral-950 hover:bg-neutral-850 text-neutral-400 hover:text-white py-2 px-4 rounded text-xs uppercase tracking-wider font-bold border border-neutral-800 transition-all cursor-pointer font-mono">Kembali ke Daftar</a>
    </div>

    <?php if(isset($errors) && $errors->any()): ?>
        <div class="bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded p-3 font-mono mb-5"><?php echo e($errors->first()); ?></div>
    <?php endif; ?>

    <form method="POST" action="<?php echo e($isEdit ? route('admin.articles.update', $article) : route('admin.articles.store')); ?>" enctype="multipart/form-data" class="space-y-6">
        <?php echo csrf_field(); ?>
        <?php if($isEdit): ?>
        ...
        <div class="bg-neutral-950 border border-blue-500/15 rounded-2xl p-5">
            <div class="flex items-center gap-1.5 text-blue-400 text-xs font-bold mb-1.5 uppercase tracking-wider font-mono">Asisten Penulisan Artikel Otomatis AI (<?php echo e($activeAiLabel); ?>)</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="category" x-model="category" class="w-full bg-neutral-950 border border-neutral-800 focus:border-blue-500/50 rounded py-2 px-3 text-xs text-white focus:outline-none">
                    <?php $__currentLoopData = \App\Http\Controllers\ArticleController::CATEGORIES; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-800 font-mono">
            <a href="<?php echo e(route('admin.index')); ?>" class="bg-neutral-950 hover:bg-neutral-850 text-neutral-400 hover:text-white font-bold py-2.5 px-4 rounded text-xs uppercase tracking-wider transition-colors cursor-pointer">Batalkan</a>
...
function adminArticleForm() {
    return {
        title: <?php echo json_encode(old('title', $article->title), 512) ?>,
        summary: <?php echo json_encode(old('summary', $article->summary), 512) ?>,
        content: <?php echo json_encode(old('content', $article->content), 512) ?>,
        category: <?php echo json_encode(old('category', $article->category ?: 'Incident Response'), 512) ?>,
        author: <?php echo json_encode(old('author', $article->author), 512) ?>,
        imageUrl: <?php echo json_encode(old('image_url', $article->image_url), 512) ?>,
        tagsInput: <?php echo json_encode($initialTagsInput, 15, 512) ?>,
        activeAiProvider: <?php echo json_encode($activeAiProvider, 15, 512) ?>,
        activeAiLabel: <?php echo json_encode($activeAiLabel, 15, 512) ?>,
        aiKeyword: '',
        aiLoading: false,
        aiError: '',
        aiSuccess: '',
        ...
        async generateDraftBackend() {
            const response = await fetch('<?php echo e(route('admin.gemini.generate-draft')); ?>', {
                method: 'POST',
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/vol15_2/infinityfree.com/if0_42129557/htdocs/resources/views/admin/form.blade.php ENDPATH**/ ?>